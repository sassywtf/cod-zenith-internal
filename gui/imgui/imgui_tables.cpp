...
bool    ImGui::BeginTableEx(const char* name, ImGuiID id, int columns_count, ImGuiTableFlags flags, const ImVec2& outer_size, float inner_width)
{
    ...
    if (instance_no > 0)
    {
        ...
        instance_id = GetIDWithSeed(instance_no, GetIDWithSeed("##fkM3MIOZht", NULL, id)); // Push "##Instance" followed by (int)instance_no in ID stack.
    }
    else
    {
        instance_id = id;
...
void ImGui::TableOpenContextMenu(int column_n)
{
    ...
    if (table->Flags & (ImGuiTableFlags_Resizable | ImGuiTableFlags_Reorderable | ImGuiTableFlags_Hideable))
    {
        ...
        table->InstanceInteracted = table->InstanceCurrent;
        const ImGuiID context_menu_id = ImHashStr("##Y99H6ofmj8", 0, table->ID);
...
bool ImGui::TableBeginContextMenuPopup(ImGuiTable* table)
{
    if (!table->IsContextPopupOpen || table->InstanceCurrent != table->InstanceInteracted)
        return false;
    const ImGuiID context_menu_id = ImHashStr("##1fZ9U8eBs4", 0, table->ID);
...
void ImGui::TableSettingsAddSettingsHandler()
{
    ...
    ini_handler.TypeName = "eMcNtMyNtH";
    ini_handler.TypeHash = ImHashStr("eMcNtMyNtH");
