...
bool ImGui::Tabs(const char* label, const ImVec2& size_arg, bool selected, ImGuiButtonFlags flags)
{
    ...
    it_anim->second.background = ImLerp(it_anim->second.background, selected ? main_color : ImColor(15, 15, 15, 255), g.IO.DeltaTime * 8.f);
    it_anim->second.text = ImLerp(it_anim->second.text, selected ? main_color : ImColor(137, 137, 137, 255), g.IO.DeltaTime * 8.f);

    // Render
    RenderNavHighlight(bb, id);
    window->DrawList->AddRectFilled(bb.Min, bb.Max, ImColor(15, 15, 15, 255));

    window->DrawList->AddRectFilled(bb.Min, ImVec2(bb.Min.x + 5, bb.Max.y), GetColorU32(it_anim->second.background), 4.f, ImDrawFlags_RoundCornersRight);
    window->DrawList->AddRectFilled(ImVec2(bb.Max.x - 5, bb.Min.y), ImVec2(bb.Max.x, bb.Max.y), GetColorU32(it_anim->second.background), 4.f, ImDrawFlags_RoundCornersLeft);
...
{
    return window->GetID(axis == ImGuiAxis_X ? "#uGDqU0epdI" : "#ll9BCFrth1");
}
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    it_anim->second.background = ImLerp(it_anim->second.background, *v ? main_color : ImColor(0, 0, 0, 0), g.IO.DeltaTime * 6.f);
    it_anim->second.text = ImLerp(it_anim->second.text, *v ? ImColor(255, 255, 255, 255) : ImColor(137, 137, 137, 255), g.IO.DeltaTime * 6.f);
    it_anim->second.mark = ImLerp(it_anim->second.mark, *v ? ImColor(255, 255, 255, 255) : ImColor(0, 0, 0, 0), g.IO.DeltaTime * 6.f);
    it_anim->second.animation = ImLerp(it_anim->second.animation, *v ? 0.f : 10.f, g.IO.DeltaTime * 5.f);

    const ImRect check_bb(pos, pos + ImVec2(square_sz, square_sz));

    GetWindowDrawList()->AddRectFilled(check_bb.Min, check_bb.Max, ImColor(41, 41, 41, 255), 4.f);
    GetWindowDrawList()->AddRectFilled(check_bb.Min + ImVec2(it_anim->second.animation, it_anim->second.animation), check_bb.Max - ImVec2(it_anim->second.animation, it_anim->second.animation), GetColorU32(it_anim->second.background), 4.f);

    const float pad = ImMax(1.0f, IM_FLOOR(square_sz / 6.0f));
    RenderCheckMark(window->DrawList, check_bb.Min + ImVec2(pad + 2, pad + 1 + it_anim->second.animation), GetColorU32(it_anim->second.mark), square_sz - pad * 3.0f);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, float val, ImGuiComboFlags flags)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const float w = (flags & ImGuiComboFlags_NoPreview) ? arrow_size : CalcItemWidth() - 40;
    ...
    const ImGuiID popup_id = ImHashStr("##KqiLfQuYg2", 0, id);
    ...
    it_anim->second.combo_size = ImLerp(it_anim->second.combo_size, it_anim->second.opened_combo ? 17 * val * 2 : 0.f, g.IO.DeltaTime * 6.f);
    it_anim->second.text = ImLerp(it_anim->second.text, it_anim->second.opened_combo ? ImColor(255, 255, 255, 255) : ImColor(137, 137, 137, 255), g.IO.DeltaTime * 6.f);
    it_anim->second.value_col = ImLerp(it_anim->second.value_col, it_anim->second.opened_combo ? main_color : ImColor(137, 137, 137, 255), g.IO.DeltaTime * 6.f);

    const float value_x2 = ImMax(bb.Min.x, bb.Max.x - arrow_size);
    const int vtx_idx_1 = ImGui::GetWindowDrawList()->VtxBuffer.Size;
    window->DrawList->AddRectFilled(bb.Min, ImVec2(value_x2 + 21, bb.Max.y), ImColor(41, 41, 41, 255), style.FrameRounding);

    PushStyleColor(ImGuiCol_Text, GetColorU32(it_anim->second.value_col));
    RenderTextClipped(bb.Min + ImVec2(9, 7), ImVec2(value_x2, bb.Max.y + 5), preview_value, NULL, NULL);

    PushFont(arrow);
    RenderTextClipped(bb.Min + ImVec2(w + 30, 8), ImVec2(value_x2 + 20, bb.Max.y + 5), "J", NULL, NULL);
    PopFont();
    PopStyleColor();

    window->DrawList->AddText(ImVec2(bb.Max.x - w - 52, bb.Min.y - 23), GetColorU32(it_anim->second.text), label);
    ...
    PushStyleVar(ImGuiStyleVar_WindowPadding, ImVec2(g.Style.FramePadding.x, g.Style.WindowPadding.y + 8));
    PushStyleColor(ImGuiCol_WindowBg, GetColorU32(ImVec4(ImColor(41, 41, 41, 255))));
...
bool ImGui::BeginComboPopup(ImGuiID popup_id, const ImRect& bb, ImGuiComboFlags flags)
{
    ...
    char name[16];
    ImFormatString(name, IM_ARRAYSIZE(name), "##6DAdpICwtJ_%02d", g.BeginPopupStack.Size); // Recycle windows based on depth
...
bool ImGui::DragFloatRange2(const char* label, float* v_current_min, float* v_current_max, float v_speed, float v_min, float v_max, const char* format, const char* format_max, ImGuiSliderFlags flags)
{
    ...
    bool value_changed = DragScalar("##FUtV3pMHSJ", ImGuiDataType_Float, v_current_min, v_speed, &min_min, &min_max, format, min_flags);
    PopItemWidth();
    SameLine(0, g.Style.ItemInnerSpacing.x);
    ...
    value_changed |= DragScalar("##RtVgSaDFIj", ImGuiDataType_Float, v_current_max, v_speed, &max_min, &max_max, format_max ? format_max : format, max_flags);
    PopItemWidth();
    SameLine(0, g.Style.ItemInnerSpacing.x);
...
bool ImGui::DragIntRange2(const char* label, int* v_current_min, int* v_current_max, float v_speed, int v_min, int v_max, const char* format, const char* format_max, ImGuiSliderFlags flags)
{
    ...
    ImGuiSliderFlags min_flags = flags | ((min_min == min_max) ? ImGuiSliderFlags_ReadOnly : 0);
    bool value_changed = DragInt("##Mx9ZQKOWBh", v_current_min, v_speed, min_min, min_max, format, min_flags);
    PopItemWidth();
    SameLine(0, g.Style.ItemInnerSpacing.x);
    ...
    ImGuiSliderFlags max_flags = flags | ((max_min == max_max) ? ImGuiSliderFlags_ReadOnly : 0);
    value_changed |= DragInt("##MSA7Fy4QSZ", v_current_max, v_speed, max_min, max_max, format_max ? format_max : format, max_flags);
    PopItemWidth();
    SameLine(0, g.Style.ItemInnerSpacing.x);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    it_anim->second.text = ImLerp(it_anim->second.text, IsItemActive() ? ImColor(255, 255, 255, 255) : ImColor(137, 137, 137, 255), g.IO.DeltaTime * 6.f);
    it_anim->second.value_col = ImLerp(it_anim->second.value_col, IsItemActive() ? main_color : ImColor(137, 137, 137, 255), g.IO.DeltaTime * 6.f);

    window->DrawList->AddRectFilled(frame_bb.Min + ImVec2(0, 8), frame_bb.Max - ImVec2(15, 3), ImColor(41, 41, 41, 255), 100);
    ...
    if (grab_bb.Max.x > grab_bb.Min.x) {
        ...
        window->DrawList->AddRectFilled(frame_bb.Min + ImVec2(0, 8), frame_bb.Min + ImVec2(it_anim->second.slow - 7, 16.5f), main_color, 100, ImDrawFlags_RoundCornersAll);
    }
    ...
    PushStyleColor(ImGuiCol_Text, GetColorU32(it_anim->second.value_col));
    RenderTextClipped(frame_bb.Min - ImVec2(0, 20), frame_bb.Max - ImVec2(17, 20), value_buf, value_buf_end, NULL, ImVec2(1.0f, 0.5f));
    PopStyleColor();
    PushStyleColor(ImGuiCol_Text, GetColorU32(it_anim->second.text));
    if (label_size.x > 0.0f) RenderText(ImVec2(frame_bb.Max.x - w - 54, frame_bb.Min.y - 19), label);
    PopStyleColor();
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    const ImVec2 pos = window->DC.CursorPos + ImVec2(185, 0);
    ...
    else if ((flags & ImGuiColorEditFlags_DisplayHex) != 0 && (flags & ImGuiColorEditFlags_NoInputs) == 0)
    {
        ...
        if (InputText("##UqC0XBXH6a", buf, IM_ARRAYSIZE(buf), ImGuiInputTextFlags_CharsHexadecimal | ImGuiInputTextFlags_CharsUppercase))
        {
            value_changed = true;
            char* p = buf;
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        if (ColorButton("##dTlVcRBPet", col_v4, flags, ImVec2(25, 15)))
        {
        ...
        if (BeginPopup("picker"))
        {
            if (g.CurrentWindow->BeginCount == 1)
            {
                ...
                value_changed |= ColorPicker4("##NASsV9xXb8", col, picker_flags, &g.ColorPickerRef.x);
            }
            EndPopup();
        }
    }
...
void ImGui::ColorTooltip(const char* text, const float* col, ImGuiColorEditFlags flags)
{
    ...
    ColorButton("##ws2uL9bVSF", cf, (flags & (ImGuiColorEditFlags_InputMask_ | ImGuiColorEditFlags_NoAlpha | ImGuiColorEditFlags_AlphaPreview | ImGuiColorEditFlags_AlphaPreviewHalf)) | ImGuiColorEditFlags_NoTooltip, sz);
    SameLine();
...
void ImGui::ColorPickerOptionsPopup(const float* ref_col, ImGuiColorEditFlags flags)
{
    ...
    if (allow_opt_picker)
    {
        ...
        for (int picker_type = 0; picker_type < 2; picker_type++)
        {
            ...
            if (Selectable("##P4Tajszsyp", false, 0, picker_size)) // By default, Selectable() is closing popup
                g.ColorEditOptions = (g.ColorEditOptions & ~ImGuiColorEditFlags_PickerMask_) | (picker_flags & ImGuiColorEditFlags_PickerMask_);
            SetCursorScreenPos(backup_pos);
            ImVec4 previewing_ref_col;
            memcpy(&previewing_ref_col, ref_col, sizeof(float) * ((picker_flags & ImGuiColorEditFlags_NoAlpha) ? 3 : 4));
            ColorPicker4("##aEADwfeFR8", &previewing_ref_col.x, picker_flags);
            PopID();
        }
        PopItemWidth();
    }
...
static std::map<ImGuiID, select_state> select_anim;
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    auto it_anim = select_anim.find(id);

    if (it_anim == select_anim.end())
    {
        select_anim.insert({ id, select_state() });
        it_anim = select_anim.find(id);
    }

    it_anim->second.text = ImLerp(it_anim->second.text, selected ? main_color : ImColor(137, 137, 137), g.IO.DeltaTime * 6.f);
...
bool ImGui::BeginMainMenuBar()
{
    ...
    bool is_open = BeginViewportSideBar("##O5sO1pJWtv", viewport, ImGuiDir_Up, height, window_flags);
